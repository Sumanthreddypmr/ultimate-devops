...
app.get('/appointments/patient/:patientId', (req, res) => {
  const patientAppointments = appointments.filter(appt => appt.patientId === req.params.patientId);
  if (patientAppointments.length > 0) {
    res.json({ message: `Found ${patientAppointments.length} appointment(s) for patient ${req.params.patientId}`, appointments: patientAppointments });
  } else {
    res.status(404).json({ message: `No appointments found for patient ${req.params.patientId}` });
  }
});
